...
if ($datosContrato['id_tipo_compra'] == FINANCIADO) {
    ...
    if ($input_concepto == APARTADO || $input_concepto == ENGANCHE) {
            ...
            $abonado_interes = 0;
            $no_mensualidad = 0;
            $id_estatus_pago = calculaIdDiferencia($inp_diferencia);
    ...
    if ($input_concepto == MENSUALIDAD_CONTRATO) {
        ...
        if ($inp_cpagada < $inp_mensualidad && $inp_recargo<0) {
            ...
            if ($aFavor >= $inp_mensualidad) {
                ...
                $diferencia = $aFavor - $inp_mensualidad;
            }else{
                $inp_cpagada = $aFavor;
                $diferencia = $inp_mensualidad - $aFavor;
            }
            ...
            $abonos = calculaAbonadoInteres($balance, 8, $inp_cpagada);

            $balance_final = $balance - $abonos['capital'];

            $id_estatus_pago = calculaIdDiferencia($inp_diferencia);

            $estatus_contrato = $datosContrato['id_estatus_venta'];

            $guardadoPago = guardaPago($id_contrato,$inp_fpago, $inp_cuenta,$no_mensualidad,$inp_cpagada,$inp_divisa,$inp_tipocambio,$cant_inicial,$abonos['capital'],$abonos['interes'],$inp_diferencia,$id_estatus_pago,$inp_comentario,$input_concepto,$inp_mensualidad,$fecha_mensualidad,$balance_final,$inp_formapago,$estatus_contrato);
        }
...
if ($datosContrato['id_tipo_compra'] == MSI) {
    ...
    if ($inp_diferencia < 0) {
        $totalAFavor = $inp_diferencia - $inp_cpagada;
        if ($inp_mensualidad <= $inp_cpagada) {
            $abonado_capital = $inp_mensualidad;
            $diferencia = $totalAFavor + $abonado_capital;
        }
        if ($inp_mensualidad > $inp_cpagada) {
            if ($inp_mensualidad > abs($totalAFavor)) {
                $abonado_capital = abs($totalAFavor);  
            }else{
                $abonado_capital = $inp_mensualidad;
            }
           // $abonado_capital = $inp_cpagada;  
...
function calculaAbonadoInteres($balanceFinal, $tasaInteres, $cantidadPagada){
    $abonos = [];
